...
function staticServer(root, onTagMissedCallback, htmlReplaceFilters) {
	var isFile = false;
	...
	return function (req, res, next) {
		 ...
		 * by 张天可
		 */
		function convert(stream, res) {
			let isHtml = false
			var x = path.extname(stream.path).toLocaleLowerCase()
			if (x == ".html") {
				isHtml = true
			}
			// console.log("isHtml", isHtml, stream);
			if (isHtml && htmlReplaceFilters && Array.isArray(htmlReplaceFilters)) {
				let htmlStrArr = []
				stream.on("data", (buffer) => {
					let str = buffer.toString()
					htmlStrArr.push(str)
					// console.log("data", stream, str)
				})
				stream.on("end", () => {
					// console.log("ended", stream)
					let str = htmlStrArr.join("")
					str = str.replace(new RegExp(injectTag, "i"), GET_INJECTED_CODE() + injectTag)
					// let str = ws.getString()
					// console.log("ws", ws.getString())
					htmlReplaceFilters.forEach(({ pathMatch, replacePairs }) => {
						if (req.url.includes(pathMatch)) {
							replacePairs.forEach(([regStr, replaceStr]) => {
								str = str.replace(new RegExp(regStr, "g"), replaceStr)
							})
						}
					})
					let ss = new StringReadableStream(str)
					let contentLength = Buffer.byteLength(str)
		...
		let sendStream = send(req, reqpath, {
			root: root
		})

		// 进行额外的文件流控制 by zhangtke
		sendStream.stream = function stream(path, options) {
...
LiveServer.start = function (options, callback) {
	...
	// 添加新参数，用于替换html内容 by zhangtke
	var htmlReplaceFilters = options.htmlReplaceFilters;

	var staticServerHandler = staticServer(root, onTagMissedCallback, htmlReplaceFilters);
